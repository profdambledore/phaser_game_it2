...
function createDrop(level, locX, locY) {
	...
	if (level == 0) {
		...
		if (spawnChance <= 10) {
			...
			speedDrop.hull.once('pointerdown', clickPickupDown);
			...
			healthDrop.hull.once('pointerdown', clickPickupDown);
			drops.push(healthDrop);
		}
	}
}

function clickPickupDown(pointer) {
	console.log('Hello Dumbass');
	//useDrop.call(this, , player.hull)
}
